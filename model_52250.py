...
X_train = data_train.iloc[:52250,:].values
...
if dele_label:
    ...
    y_train = label['label'].values[:52250]
#     y_train = label.iloc[:52250,'label'].values
...
#     predictions_lgb=[]
# X_true = pd.read_csv('B-32/B-第三天.csv',engine='python').prediction

for fold_, (trn_idx, val_idx) in enumerate(folds.split(X_train, y_train)):
    print("fold n°{}".format(fold_+1))
    trn_data = lgb.Dataset(X_train[trn_idx], y_train[trn_idx])
    val_data = lgb.Dataset(X_train[val_idx], y_train[val_idx])
#     val_data = lgb.Dataset(X_test, X_true)
    num_round = 10000
    clf = lgb.train(param, 
                    trn_data,
                    num_round, 
...
name = 'baseline-lamda=0.1-num=30-leaf10-maxdepth-0.4525-newfea+百分比error-qudiao52250'
