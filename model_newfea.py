...
data_train = pd.concat([pd.read_csv('mydata/data_train.csv'),
                        pd.read_csv('mydata/data_train_peak.csv')],axis=1)

data_test = pd.concat([pd.read_csv('mydata/data_test.csv'),
                        pd.read_csv('mydata/data_test_peak.csv')],axis=1)
...
data_test.drop('sample_file_name',inplace=True,axis=1)
...
# ## 概率baseline-lamda=0.1-num=20-leaf10-maxdepth-0.45525-newfea.csv
...
#     predictions_lgb.append(clf.predict(X_test, num_iteration=clf.best_iteration))
    if do_test==1:
...
name = 'baseline-lamda=0.1-num=20-leaf10-maxdepth-0.45525-newfea'

up_per.to_csv(file_name+'/概率'+name+'.csv',index=None)


# ## 概率baseline-lamda=0.1-num=30-leaf10-maxdepth-0.45525-newfea.csv
...
name = 'baseline-lamda=0.1-num=30-leaf10-maxdepth-0.45525-newfea'

up_per.to_csv(file_name+'/概率'+name+'.csv',index=None)
up_per.to_csv(file_name+'/second-概率'+name+'.csv',index=None)
...
data_test = pd.concat([pd.read_csv('mydata/data_test.csv'),
                        pd.read_csv('mydata/data_test_peak.csv')],axis=1)
for i in ['发动机转速_error', '油泵转速_error', '泵送压力_error', '分配压力_error',]:
    data_test[i]=data_test[i]/data_test['长度']
    data_train[i]=data_test[i]/data_test['长度']
#     data_train[i]=data_train[i]/data_train['长度']
# 删除某一类别占比超过90%的列
good_cols = list(data_train.columns)
bad_cols = []
for col in data_train.columns:
...
name = 'baseline-lamda=0.1-num=30-leaf10-maxdepth-0.4525-newfea+百分比error'
